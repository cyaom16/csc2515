...
def mogEM(x, K, iters, randConst=1, minVary=0):
    ...
    p /= np.sum(p)   # mixing coefficients
...
def q4():
    ...
    for t in xrange(T):
        ...
        p_anger, mu_anger, vary_anger, log_likelihood_train_anger = mogEM(x_train_anger, K, iters,
                                                          randConst=randConst, minVary=minVary)
        p_happy, mu_happy, vary_happy, log_likelihood_train_happy = mogEM(x_train_happy, K, iters,
                                                          randConst=randConst, minVary=minVary)
        ...
        log_likelihood_train_anger = mogLogLikelihood(p_anger, mu_anger, vary_anger, x_train) + log_likelihood_class[0]
        log_likelihood_train_happy = mogLogLikelihood(p_happy, mu_happy, vary_happy, x_train) + log_likelihood_class[1]
        predict_train = (log_likelihood_train_anger < log_likelihood_train_happy).astype(float)

        log_likelihood_valid_anger = mogLogLikelihood(p_anger, mu_anger, vary_anger, x_valid) + log_likelihood_class[0]
        log_likelihood_valid_happy = mogLogLikelihood(p_happy, mu_happy, vary_happy, x_valid) + log_likelihood_class[1]
        predict_valid = (log_likelihood_valid_anger < log_likelihood_valid_happy).astype(float)

        log_likelihood_test_anger = mogLogLikelihood(p_anger, mu_anger, vary_anger, x_test) + log_likelihood_class[0]
        log_likelihood_test_happy = mogLogLikelihood(p_happy, mu_happy, vary_happy, x_test) + log_likelihood_class[1]
        predict_test = (log_likelihood_test_anger < log_likelihood_test_happy).astype(float)

        errorTrain[t] = np.sum((predict_train != y_train).astype(float)) / y_train.shape[0]
        errorValidation[t] = np.sum((predict_valid != y_valid).astype(float)) / y_valid.shape[0]
        errorTest[t] = np.sum((predict_test != y_test).astype(float)) / y_test.shape[0]
